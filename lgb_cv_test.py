...
for index,(train_index,test_index) in enumerate(skf.split(X,y)):
    ...
    X_train,X_valid,y_train,y_valid,w_train,w_valid = X[train_index],X[test_index],y[train_index],y[test_index],y_weights[train_index],y_weights[test_index]

    train_data = lgb.Dataset(X_train, label=y_train,weight=w_train)
    validation_data = lgb.Dataset(X_valid, label=y_valid,weight=w_valid)

    clf=lgb.train(params,train_data,num_boost_round=500,valid_sets=[validation_data],early_stopping_rounds=50,feval=f1_score_vali,verbose_eval=1)
